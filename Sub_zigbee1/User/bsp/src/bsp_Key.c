...
*********************************************************************************************************
*/
/* 为了区分6个事件：WKUP单独按下，KEY0单独按下，KEY1单独按下，KEY2单独按下，WKUP和KEY0同时按下，KEY0和KEY1同时按下 */
...
static void bsp_InitKeyHard(void)
{
	...
	//4.调用GPIO_Init
	GPIO_Init(WKUP_GPIO_PORT,&GPIO_InitStructure);	//WKUP初始化
	...
	//4.调用GPIO_Init
	GPIO_Init(KEY0_GPIO_PORT,&GPIO_InitStructure);	//KEY0初始化
	
	//KEY1-------------------------------------------------------------------------------------
	//3.结构体赋值
	GPIO_InitStructure.GPIO_Pin = KEY1_GPIO_PIN;	//KEY1引脚
	GPIO_InitStructure.GPIO_Mode = KEY1_GPIO_MODE;	//KEY1端口模式
	//4.调用GPIO_Init
	GPIO_Init(KEY1_GPIO_PORT,&GPIO_InitStructure);	//KEY1初始化
	
	//KEY2-------------------------------------------------------------------------------------
	//3.结构体赋值
	GPIO_InitStructure.GPIO_Pin = KEY2_GPIO_PIN;	//KEY2引脚
	GPIO_InitStructure.GPIO_Mode = KEY2_GPIO_MODE;	//KEY2端口模式
	//4.调用GPIO_Init
	GPIO_Init(KEY2_GPIO_PORT,&GPIO_InitStructure);	//KEY2初始化
...
//		s_tBtn[i].KeyCodeLong	= 3 * i + 3;			/* 按键被持续按下的简直代码 */
		s_tBtn[i].RepeatSpeed = 0;						/* 按键连发的速度，0表示支持连发 */
