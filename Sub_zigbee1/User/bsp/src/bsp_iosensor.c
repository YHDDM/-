...
uint8_t FIRE_OUT_flag = 0;  	//火焰传感器触发标志
uint8_t HALL_OUT_flag = 0;  	//霍尔传感器触发标志
uint8_t GASES_OUT_flag = 0;  	//可燃气体传感器触发标志
///////////////////////////////////////
/* 外部变量申明区 */
...
//	RCC_APB2PeriphClockCmd(FIRE_OUT_GPIO_CLK,ENABLE); //使能FIRE_OUT时钟 
//	//初始化 FIRE_OUT    
//	GPIO_InitStructure.GPIO_Pin  = FIRE_OUT_GPIO_PIN; 
//	GPIO_InitStructure.GPIO_Mode = FIRE_OUT_GPIO_MODE; //设置成输入，默认上拉    
//	GPIO_Init(FIRE_OUT_GPIO_PORT, &GPIO_InitStructure); //初始化FIRE 
	
//	RCC_APB2PeriphClockCmd(GASES_OUT_GPIO_CLK,ENABLE); //使能VIBRATE_OUT时钟 
//	//初始化 GASES_OUT    
//	GPIO_InitStructure.GPIO_Pin  = GASES_OUT_GPIO_PIN; 
//	GPIO_InitStructure.GPIO_Mode = GASES_OUT_GPIO_MODE; //设置成输入，默认上拉    
//	GPIO_Init(GASES_OUT_GPIO_PORT, &GPIO_InitStructure); //初始化GASES 
	
//	RCC_APB2PeriphClockCmd(HALL_OUT_GPIO_CLK,ENABLE); //使能HALL_OUT时钟 
//	//初始化 HALL_OUT    
//	GPIO_InitStructure.GPIO_Pin  = HALL_OUT_GPIO_PIN; 
//	GPIO_InitStructure.GPIO_Mode = HALL_OUT_GPIO_MODE; //设置成输入，默认上拉    
//	GPIO_Init(HALL_OUT_GPIO_PORT, &GPIO_InitStructure); //初始化HALL 
...
//	//FIRE中断线以及中断初始化配置  
//	GPIO_EXTILineConfig(FIRE_OUT_PORT_SRC,FIRE_OUT_PIN_SRC); 
//	
...
//	EXTI_Init(&EXTI_InitStructure);  //根据EXTI_InitStruct中指定的参数初始化外设EXTI寄存器
//	
//	NVIC_InitStructure.NVIC_IRQChannel = FIRE_OUT_EXTI_IRQN; 	//使能FIRE所在的外部中断通道 
//	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x02;  	//抢占优先级2  
//	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x02;  			//子优先级2  
//	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;  				//使能外部中断通道 
//	NVIC_Init(&NVIC_InitStructure);     	//根据NVIC_InitStruct中指定的参数初始化外设NVIC寄存器
...
//	//HALL中断线以及中断初始化配置  
//	GPIO_EXTILineConfig(HALL_OUT_PORT_SRC,HALL_OUT_PIN_SRC); 
//	
//	EXTI_InitStructure.EXTI_Line = HALL_OUT_EXTI_LINE; 
//	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;  
//	EXTI_InitStructure.EXTI_Trigger = HALL_OUT_EXTI_TRIG; 
//	EXTI_InitStructure.EXTI_LineCmd = ENABLE; 
//	EXTI_Init(&EXTI_InitStructure);  //根据EXTI_InitStruct中指定的参数初始化外设EXTI寄存器
//	
//	NVIC_InitStructure.NVIC_IRQChannel = HALL_OUT_EXTI_IRQN; 	//使能HALL所在的外部中断通道 
//	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x02;  	//抢占优先级2  
//	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x02;  			//子优先级2  
//	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;  				//使能外部中断通道 
//	NVIC_Init(&NVIC_InitStructure);     	//根据NVIC_InitStruct中指定的参数初始化外设NVIC寄存器
...
//	//FIRE中断线以及中断初始化配置  
//	GPIO_EXTILineConfig(GASES_OUT_PORT_SRC,GASES_OUT_PIN_SRC); 
//	
//	EXTI_InitStructure.EXTI_Line = GASES_OUT_EXTI_LINE; 
//	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;  
//	EXTI_InitStructure.EXTI_Trigger = GASES_OUT_EXTI_TRIG; 
//	EXTI_InitStructure.EXTI_LineCmd = ENABLE; 
//	EXTI_Init(&EXTI_InitStructure);  //根据EXTI_InitStruct中指定的参数初始化外设EXTI寄存器
//	
//	NVIC_InitStructure.NVIC_IRQChannel = GASES_OUT_EXTI_IRQN; 	//使能GASES所在的外部中断通道 
//	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x02;  	//抢占优先级2  
//	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x02;  			//子优先级2  
//	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;  				//使能外部中断通道 
//	NVIC_Init(&NVIC_InitStructure);     	//根据NVIC_InitStruct中指定的参数初始化外设NVIC寄存器
...
//	{
//		if(FIRE_OUT==FIRE_OUT_ACTIVE_LEVEL) //FIRE_OUT动作为低电平
//		{
//			FIRE_OUT_flag=1;	//标志置位，便于主函数轮询 
//		}
//		//中断处理，自行添加
...
//	{
//		if(HALL_OUT==HALL_OUT_ACTIVE_LEVEL) //HALL_OUT动作为低电平
//		{
//			HALL_OUT_flag=1;	//标志置位，便于主函数轮询 
//		}
//		//中断处理，自行添加
//		EXTI_ClearITPendingBit(HALL_OUT_EXTI_LINE);
//	}
//	if(EXTI_GetITStatus(GASES_OUT_EXTI_LINE)!=RESET)
//	{
//		if(GASES_OUT==GASES_OUT_ACTIVE_LEVEL) //GASES_OUT动作为低电平
//		{
//			GASES_OUT_flag=1;	//标志置位，便于主函数轮询 
//		}
//		//中断处理，自行添加
//		EXTI_ClearITPendingBit(GASES_OUT_EXTI_LINE);
//	}
