...
*	功能说明: 读出OPT3001内部数据,连续两个
*	形    参： uint8_t ST_Address:OPT3001寄存器地址
*	返 回 值:  uint16_t 数据 
*********************************************************************************************************
*/
...
*	功能说明: 获取OPT3001的设备ID
*	形    参：无
*	返 回 值:  uint16_t OPT3001的设备ID 初始值为0x3001 
*********************************************************************************************************
*/
...
{ 
	...
	OPT3001Config();     //配置OPT3001 并且唤醒OPT3001 
	delay_ms(5); 
	...
	if( (ManufacturerIDNum == OPT3001_ManufacturerID ) && ( DeviceIDNum == OPT3001_DeviceID ) )  //如果读出来的ID都正确表示初始化成功 
	{ 
		return 0; 
	} 
	else   //否则初始化不成功 
...
*	形    参：无
*	返 回 值: 无
*	注意：12-15位 RN[0,3]:0x1100,设置为Full-Scale Mode 
*   9-10：M[0,1]:0x10,设置OPT3001为连续转换模式 
