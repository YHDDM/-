...
FSMC_NORSRAMInitStructure.FSMC_Bank = FSMC_Bank1_NORSRAMx;// 这 里 我 们 使 用 NE4 ， 也 就 对 应
...
/*
*********************************************************************************************************
*	函 数 名: LCD_ShowChinese
*	功能说明: 显示中文汉字，需自行在汉字取模软件中中取模
*	形    参：x,y坐标，num文字下标，size字体大小：16
				mode:	[7]:0,不填充;1,填充0.
*						[6:1]:保留
*						[0]:0,非叠加显示;1,叠加显示.
*	返 回 值: 无
...
/*
*********************************************************************************************************
*	函 数 名: LCD_ShowPicture
*	功能说明: 显示图片，需自行在图片取模软件中中取模
*	形    参：x,y坐标，column图片 长，row图片 宽，注意，需要查看图片属性像素点，例如像素点为235×320，则column和row的数值也为235和320，必须一致，不然会出现显示错误
				mode:	[7]:0,不填充;1,填充0.
*						[6:1]:保留
*						[0]:0,非叠加显示;1,叠加显示.
*	返 回 值: 无
